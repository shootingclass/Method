...
class LinearProbeLightningModule(pl.LightningModule):
    ...
    def forward(self, sensor_data):
        ...
        if self.hparams.model_name == "imu2clip":
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.test_auroc.update(probs, y) # AUC는 확률값(probs) 사용
        self.test_ap.update(probs, y)    # AP는 확률값(probs) 사용

        self.log("test_loss", loss, on_epoch=True)
        self.log("test_acc", self.test_accuracy, on_epoch=True)
        self.log("test_f1_micro", self.test_f1_micro, on_epoch=True)
        self.log("test_f1_macro", self.test_f1_macro, on_epoch=True)
        self.log("test_f1_weighted", self.test_f1_weighted, on_epoch=True)
...
if __name__ == "__main__":
    ...
    # --- 모델/학습 관련 인자 ---
    parser.add_argument('--linear_epochs', type=int, default=20, help='Number of epochs for linear probing.')
