...
class PRIMUSLightningModule(BasePretrainModule):
    def __init__(self, args):
        ...
        self.num_views = 2
        self.ssl_coeff = args.ssl_coeff
        self.nnclr = args.nnclr
        ...
        self.transform_indices = args.transform_indices
        self.sensor_transform = generate_combined_transform_function(sensor_transforms, self.transform_indices)
        ...
        self.sensor_model = MW2StackRNNPoolingMultihead(num_sensors=args.num_sensors, size_embeddings=self.hparams.embedding_dim)
        self.video_model = Clip4CLIPModel(freeze=True)

    def setup(self, stage: str):
        if self.nnclr and stage == 'fit':
    ...
    def forward(self, batch, train_time=False):
        ...
        x_video = videos
        y_video = torch.zeros((len(batch), 512), dtype=torch.float32).to(self.device)
