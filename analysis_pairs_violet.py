...
output_dir = "/home/jaemo/z_tsne/boxplots_epoch_debug_violet"
...
for fname in sorted(os.listdir(input_dir)):
    ...
    try:
        ...
        # ==========================================================
        # ★★★ 박스 플롯을 바이올린 플롯으로 교체한 부분 ★★★
        # ==========================================================
        plt.figure(figsize=(6,6))

        # plt.violinplot 호출 및 옵션 설정
        parts = plt.violinplot(
            [spatial_sims, temporal_sims],
            positions=[1, 2],       # x축 위치 설정
            showmeans=False,        # 평균 표시 X
            showextrema=True,       # 최소/최대값 표시
            showmedians=True,       # 중앙값 표시
            widths=0.8
        )
        ...
        for i, color in enumerate(colors):
            ...
            parts['bodies'][i].set_edgecolor(color)
            parts['bodies'][i].set_alpha(0.4)
        
        # 중앙값 (Median) 선 색상 설정 (모든 중앙값이 한 객체로 설정됨)
        parts['cmedians'].set_color('black')
        
        # 축 라벨 설정 (boxplot의 tick_labels 대신 xticks 사용)
        ...
        # 파일 이름에 'violinplot' 반영
        out_path = os.path.join(output_dir, f"{epoch_name}_violinplot_cosine.png")
